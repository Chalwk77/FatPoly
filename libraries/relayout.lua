...
local function relayout()
    for i = #_M.relayoutListeners, 1, -1 do
        ...
        elseif type(l) == 'table' then
            -- Listener is a display object
            if not l.x then
                -- Remove removeSelf()'ed objects from the list
                table.remove(_M.relayoutListeners, i)
            else
                if type(l.relayout) == 'function' then
                    -- Object has object:relayout() function
                    l:relayout()
                else
                    -- If not, perform automatic repositioning
                    local r = l._relayout
                    if r.anchor then
                        -- Which screen corner is the object positioned closer to
...
function _M.add(listener)
    ...
    if type(listener) == 'table' and not listener.relayout then
        ...
        for anchorX = 0, 1, 0.5 do
            -- Iterate through all screen corners and edges and find the closest one
...
function _M.remove(listener)
    if not listener or not _M.relayoutListeners then
        return
    end
